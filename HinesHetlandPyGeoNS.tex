...
\end{equation}
The frequency response for eq. (\ref{eq:NDSolution}) can once again be recognized as a low-pass filter with cutoff frequency $\omega_c$.  In the limit as $n \to \infty$, the frequency response becomes a $d$-dimensional box which is zero for all the frequency tuples which have at least one component whos magnitude is greater than $\omega_c$.  It can be verified through eigen decomposition that eq. (\ref{eq:NDSolution}) behaves as a low-pass filter with cutoff frequency $\omega_c$ even when the idealized conditions are not satisfied.   
