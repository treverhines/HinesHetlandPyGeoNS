...
The RBF-FD method was introduced as an efficient way to solve partial differential equations over arbitrarily shaped multi-dimensional problem domains. When compared to alternative strategies such as the finite element method, the RBF-FD method has demonstrated superior accuracy while also being substantially easier to implement.  

In this paper we demonstrate using the RBF-FD method to calculate strain rates from geodetic data in a two step process. First we demonstate using the RBF-FD method to spatially smooth the velocity field. Our smoothing strategy can be viewed  as low-pass filter for scattered data where the degree of smoothness is controlled by a user specifies cutoff frequency.  In the second step we use the RBF-FD method to differentiate the smoothed velocity field.  We also demonstrate that this process can be used to estimate a time dependent strain rates.  In that case, we temporally smooth and differentiate GPS time series to get a time dependent velocity field.  We then spatially smooth and differentiate the resulting velocity field to get a time dependent strain map. Our method is computationally efficient and stable (there is no inversion of an ill conditioned matrix). It also does not require tuning an intangible penalty parameter, and instead just requires the user to specify a minimum spatial wavelength for the underlying geophysical signal.  Because of the simplicity of out method, it can easily be extended to time dependent estimates of strain by independently estimating strain for velocity fields for different time epochs. Additionally, our method can be easily extending to account for known discontinuities in the displacement field from, for example, a creeping fault.  

mention the touted accuracy of RBFs and where they came from
mention how we smooth then diff
mention Howell2016
