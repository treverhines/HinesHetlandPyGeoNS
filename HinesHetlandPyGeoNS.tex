...
We must also elaborate on the stencil size, $n$, and the polynomial order $m$.  We choose $m$ to be equal to the order of the derivative which we are approximating.  This choice is based on the analysis of \citet{Flyer2016} and it ensures that eq. (\ref{eq.RBFFDApprox}) will converge to the true derivative as the distance between nodes decreases.  The accuracy of eq. (\ref{eq.RBFFDApprox}) also generally increases with larger values of $n$, but at the expense of computation costs.  We then choose $n$ to be just large enough so that larger values in $n$ do not produce noticable changes in the derivative approximation. We find that $n=30$ is an appropriate choice for the demonstrations in this paper.  It is worth noting that it is not possible to invert eq. (\ref{eq.RBFFDWeights}) when the number of nodes is less than the number of monomial basis functions, and so $n$ must be greater than or equal to ${{p+d}\choose{d}}$.     
