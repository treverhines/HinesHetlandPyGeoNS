...
We discuss a method to smooth observed data, $u_\mathrm{obs}$, which can be spatial, temporal, or higher dimensional data, and the observations are generally irregularly spaced.  There are two well established catagories of methods for solving this task.  The first catagory is regularized least squares fitting of radial basis functions (RBF's) to the observation \citep[e.g.][]{Fasshauer2007}. Smoothing splines are a notable member of this catagory.  The second catagory is Gaussian process regression \citep[e.g.][]{Rasmussen2006}, which encompasses Kriging.  Our method is an example of Gaussian process regression where our smoothed solution, $u_\mathrm{post}$, incorporates $u_\mathrm{obs}$ and a stochastic prior model for the underlying signal which we are trying to recover.  We constrain $u_\mathrm{post}$ with the observation equation
