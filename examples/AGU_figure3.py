...
# smooth the data
D = rbf.fd.weight_matrix(x,x,diffs=[[2,0],[0,2]],vert=vert,smp=smp,n=20).toarray()
...
k = Quiver(axs[0],[-1.0],[-1.1],[0.5],[0.0],sigma=([0.1],[0.1],[0.0]),
           scale=2.0,width=0.005,color=(0.2,0.2,1.0),zorder=1,
           ellipse_kwargs={'edgecolors':(0.2,0.2,1.0)})
axs[0].add_collection(k)
axs[0].text(-0.65,-1.15,'$\mathregular{u_{obs}}$',fontsize=10)
k = Quiver(axs[0],[-1.0],[-1.25],[0.5],[0.0],scale_units='xy',angles='xy',
           scale=2.0,width=0.005,color='k',zorder=1)
axs[0].add_collection(k)
axs[0].text(-0.65,-1.3,'$\mathregular{u_{true}}$',fontsize=10)
axs[0].plot([-0.2,0.0],[-1.1,-1.1],'m-')           
axs[0].text(0.05,-1.15,'discontinuity',fontsize=10)           
...
k = Quiver(axs[1],[-1.0],[-1.1],[0.5],[0.0],sigma=([0.1],[0.1],[0.0]),
           scale=2.0,width=0.005,color=(0.2,0.2,1.0),zorder=1,
           ellipse_kwargs={'edgecolors':(0.2,0.2,1.0)})
axs[1].add_collection(k)
axs[1].text(-0.65,-1.15,'$\mathregular{u_{smooth}}$',fontsize=10)
k = Quiver(axs[1],[-1.0],[-1.25],[0.5],[0.0],scale_units='xy',angles='xy',
           scale=2.0,width=0.005,color='k',zorder=1)
axs[1].add_collection(k)
axs[1].text(-0.65,-1.3,'$\mathregular{u_{true}}$',fontsize=10)
axs[1].plot([-0.2,0.0],[-1.1,-1.1],'m-')           
axs[1].text(0.05,-1.15,'discontinuity',fontsize=10)           



axs[0].set_xlim((-1.25,1.25))
axs[1].set_xlim((-1.25,1.25))
axs[0].set_ylim((-1.4,1.1))
axs[1].set_ylim((-1.4,1.1))
for s in smp: axs[0].plot(vert[s,0]*w,vert[s,1]*w,'m-',lw=1,zorder=0)
for s in smp: axs[1].plot(vert[s,0]*w,vert[s,1]*w,'m-',lw=1,zorder=0)
